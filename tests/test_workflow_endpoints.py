...
def _build_mini_registry(tmp_path: Path) -> WorkflowRegistry:
    """sprite/v (template + intent + tags) + sprite/legacy_var (template 없음) +
    pixel_bg/sdxl (needs_conversion). §1.B subject + §1.C recommend/search 모두
    검증 가능한 통합 fixture.
    """
    ...
    # §1.A 사이드카 — sprite/v 는 §1.B subject 모드 + §1.C recommend/search 매칭
    # 둘 다 가능한 풀 메타. legacy_var 는 메타 없음 → legacy 모드 fallback +
    # recommend/search 결과에서 자동 제외.
...
def test_catalog_endpoint_returns_registry_shape(isolated) -> None:  # noqa: ANN001
    ...
    # 통합 사이드카 yaml 로드 — §1.A intent / tags + §1.B prompt_template 모두 채워짐
    assert v["meta"]["intent"] == "Pixel-art character sprite for retro game"
...
def test_search_endpoint_no_filter_returns_all(isolated) -> None:  # noqa: ANN001
    """필터 미지정 → 모든 available 변형 (sprite/v + sprite/legacy_var)."""
    with TestClient(server.app) as client:
        r = client.get("/api/workflows/search")
    assert r.status_code == 200
    names = {m["variant"] for m in r.json()["matches"]}
    # legacy_var 는 메타 없지만 search 는 available 만 필터하므로 포함됨
